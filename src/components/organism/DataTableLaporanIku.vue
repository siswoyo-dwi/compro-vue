<template>
	<div class="bg-white px-3 py-3 shadow-sm overflow-y-auto">
		<div class="min-h-full">
			<!-- Data table -->
			<div
				v-for="data in dis_data"
				class="datatable-container">
				<Typography v-if="data.children.length > 0">{{ data.nama }}</Typography>
				<table
					v-for="i in arrangeData(data.children)"
					:key="i.nama"
					id="datatable-search"
					class="!w-full border-collapse text-xxs mb-5">
					<thead>
						<tr>
							<th
					...
					<tbody>
						<!-- <tr :class="striped ? (idx % 2 === 0 ? 'bg-white' : 'bg-gray-50') : 'bg-white'">
							<td
								colspan="5"
...
<script setup>
	...
	const dis_data = computed(() => {
		return props.items
			.filter((item) => item.children && item.children.length > 0)
			.map((item) => {
				const filteredChildren = item.children.filter((child) => child.sumber_dana && child.sumber_dana.length > 0);
	...
	function calculateRowspan(data, key, value) {
		return data.filter((item) => item[key] === value).length;
	}

	function shouldShowRow(index, data, key) {
		return index === 0 || data[index][key] !== data[index - 1][key];
	}
